...
import { HeatmapData, getSeverityColor, getSeverityBgColor, getUniqueSubProcesses, getUniqueRootCauses } from "@/data/deviationAnalysisData";
...
export function DeviationHeatmap({ data, title = "Sub-Process vs Root Cause Heatmap" }: DeviationHeatmapProps) {
  ...
  // Get cell data
  const getCellData = (subProcess: string, rootCause: string): HeatmapData | null => {
    const key = `${subProcess}|${rootCause}`;
    return dataMap.get(key) || null;
  };

  // New improved color scheme
  const getCircleColor = (severity: HeatmapData["severity"] | null, count: number): string => {
  ...
  const getCircleBgColor = (severity: HeatmapData["severity"] | null, count: number): string => {
      ...
      <CardHeader className="pb-4 border-b border-border/50">
        <div className="flex items-center justify-between">
          ...
          <div className="hidden md:flex items-center gap-3">
            ...
            {[
              { severity: "Critical" as const, label: "Critical (8+)", color: "#EF4444", count: 8 },
              { severity: "High" as const, label: "High (5-7)", color: "#F97316", count: 6 },
              { severity: "Medium" as const, label: "Medium (2-4)", color: "#EAB308", count: 3 },
              { severity: "Low" as const, label: "Low (1)", color: "#84CC16", count: 1 },
            ].map((item) => (
              <div key={item.severity} className="flex items-center gap-1.5">
                <div
                  className="w-4 h-4 rounded-full flex items-center justify-center text-[9px] font-bold text-white"
                  style={{
                    background: `radial-gradient(circle at 30% 30%, ${hexToRgba(item.color, 1)}, ${hexToRgba(item.color, 0.85)} 50%, ${hexToRgba(item.color, 0.7)})`,
                    boxShadow: `
                ...
                >
                  {item.count >= 8 ? "8" : item.count >= 5 ? "6" : item.count >= 2 ? "3" : "1"}
                </div>
                <span
            ...
            <table className="w-full border-collapse">
              ...
              <tbody>
                {subProcesses.map((subProcess, rowIndex) => (
                  <tr 
                    key={subProcess} 
                    className={`transition-colors ${rowIndex % 2 === 0 ? "bg-white hover:bg-muted/30" : "bg-muted/10 hover:bg-muted/40"}`}
                  >
                        ...
                        >
                          {count > 0 ? (
                            <div className="flex flex-col items-center justify-center gap-1.5">
                              ...
                              <div
                                className={`${getCircleSize(count)} rounded-full flex items-center justify-center font-extrabold text-white transition-all duration-300 group-hover:scale-110 group-hover:shadow-lg cursor-pointer relative`}
                                style={{
                                  background: `radial-gradient(circle at 30% 30%, ${hexToRgba(getCircleColor(severity, count), 1)}, ${hexToRgba(getCircleColor(severity, count), 0.85)} 50%, ${hexToRgba(getCircleColor(severity, count), 0.7)})`,
                                  boxShadow: `
                                    0 4px 12px ${hexToRgba(getCircleColor(severity, count), 0.4)},
                              ...
                              {severity && (
                                <span
                                  ...
                                  style={{
                                    ...
                                    backgroundColor: hexToRgba(getCircleColor(severity, count), 0.1),
                                  }}
                                >
                                  {severity}
                                </span>
        ...
        <div className="mt-4 relative overflow-hidden rounded-xl border border-[#9909e0]/30 bg-gradient-to-br from-[#9909e0]/8 to-[#8b5cf6]/5 p-4">
          ...
          <div className="relative z-10">
            <div className="flex items-center gap-2.5 mb-3">
              ...
              <h4 className="text-sm font-bold text-foreground">Key Insights</h4>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">
              <strong className="text-[#EF4444] font-semibold">Critical concentration</strong> in{" "}
              <strong className="text-foreground">Vendor Onboarding &gt; Insufficient Process Capacity</strong> (8 occurrences) signals
              structural undersourcing requiring immediate capacity augmentation or automation. Invoice Processing shows balanced
              distribution across validation and policy gaps, indicating need for both systems controls and policy clarification.
            </p>
          </div>
        </div>
      </CardContent>
